...
def toggle_task(index: int):
    ...
    if 0 <= index < len(st.session_state.tasks):
        st.session_state.tasks[index]["completed"] = not st.session_state.tasks[index][
            "completed"
        ]
        status = (
            "completed"
        ...
        st.toast(
            f"Task '{st.session_state.tasks[index]['task']}' is now {status}!",
...
if st.session_state.tasks:
    ...
    for index, task in enumerate(st.session_state.tasks):
        ...
        with col2:
            st.button(
                "Toggle", key=f"toggle_{index}", on_click=toggle_task, args=(index,)
            )

        with col3:
            st.button(
                "Remove", key=f"remove_{index}", on_click=remove_task, args=(index,)
            )
